...
const DEMOS = [
  ...
  },
  {
    ...
    caseStudies: [
      ...
      { client: "Hisense", result: "Training duration cut from 28 to 18 days (-40%)." },
      { client: "Rogers", result: "Training duration cut from 30 to 19 days (-32%)." },
      { client: "Choice Health", result: "Training duration cut from 17 to 13 days (-24%)." },
      { client: "NCB", result: "Training duration cut from 38 to 29 days (-24%)." },
    ],
    status: "needs-review",
    note: "Source deck reused the tagline \"Faster, smarter agent training\" verbatim across this, CoBrowse, and Email AI (copy/paste artifact). Wrote a distinct tagline here. Case studies added from HGS AgentX positioning doc, then broken out into individual per-client figures (docs/Adarsh.docx, Aug 2026) instead of the earlier vague \"Hisense, Rogers, Choice Health, NCB and others, 24-40%\" aggregate. One more case in that source table (16→12 days, -25%) is left out here: its client name is still an open placeholder (\"Client F\") pending an unresolved doc comment from Adarsh Hegde asking Savita Jones to supply the real name — add it once resolved.",
  },
  {
  ...
  {
    id: "automated-disclosure-reader",
    subcategory: "employee-assist",
    title: "Automated Disclosure Reader (ADR)",
    tagline: "Every regulatory disclosure, read the same way, every time",
    industryTags: ["cross-industry", "financial-services"],
    product: "HGS AgentX",
    problem: [
      "Agents mis-read or rush lengthy regulatory disclosures, driving compliance errors and brand risk",
      "Reading disclosures accurately slows agents down and can cost them sales targets and incentives",
      "Manual disclosure delivery is inconsistent from agent to agent and hard to audit",
    ],
    whatYouSee: [
      "Speech AI plays back standardized, compliant disclosure scripts during the live call",
      "Agent is freed to focus on the conversation and the sale while the script runs",
      "Guarantees 100% accurate, compliant disclosure delivery on every call",
    ],
    variants: [{ label: "Default", video: null }],
    caseStudies: [
      { client: "Amex", result: "100% ADR compliance readout; 86.2% reduction in disclosure (Type 12) error rate; 74.3% reduction in handle time; 26% improved offer acceptance rate; 22% improvement in offer rate; 6.21% YoY reduction in agent attrition at ADR-deployed locations." },
    ],
    status: "needs-review",
    note: "New entry sourced from docs/Adarsh.docx (Aug 2026), an internal Agentic CX / Agentic Process Automation positioning doc, which names Amex as the case study behind this module. No demo video or screenshot identified yet.",
  },
  {
    id: "live-agent-assist",
    subcategory: "employee-assist",
    title: "Live Agent Assist",
    tagline: "The right next step, surfaced in the moment — not looked up after",
    industryTags: ["cross-industry"],
    product: "HGS AgentX",
    problem: [
      "Agents have to guess or go hunting for the right next step mid-conversation",
      "Inconsistent handling of the same situation across agents and shifts",
      "Coaching and knowledge tools help before or after a call, but not live, in the moment",
    ],
    whatYouSee: [
      "Live next-best-action guidance surfaced during the conversation itself",
      "Draws on the same knowledge base and interaction context as the rest of AgentX",
      "Standardizes handling of a given situation across every agent and shift",
    ],
    variants: [{ label: "Default", video: null }],
    status: "needs-review",
    note: "New entry sourced from docs/Adarsh.docx (Aug 2026), which describes this as the \"Assist\" capability within Intelligent Employee Assist: \"Live next-best-action guidance during the conversation, so agents always know the right next step instead of guessing.\" The Rogers & Shaw case study on knowledge-assist (AgentX KB) actually credits some of its results to this next-best-action guidance jointly with the knowledge base, but the doc doesn't split out figures specific to this capability alone, so no case study is attached here yet. No demo video or screenshot identified.",
  },
  {
    id: "email-ai",
    subcategory: "employee-assist",
  ...
  // ---------------- Supervisor & QA Insights ----------------
  {
    ...
    whatYouSee: [
      ...
      "GenAI Insights: ask-anything interface lets business users query performance and customer data in plain language, without waiting on a report",
    ],
    variants: [{ label: "Default", video: null }],
    caseStudies: [
      { client: "Astound", result: "GenAI Auto QA scored 100% of interactions (vs. a small manual sample); a related engagement saved 11,700 hours/year on reporting, lifted cross-selling 87%, increased agent retention 89%, and cut team-leader support costs 20%." },
      { client: "Rogers & Shaw", result: "GenAI-driven digitized quality assessment scaled coverage to up to 100% of interactions (vs. under 1% manually), cut results turnaround from 7+ days to under 24 hours, and reached 97%+ scoring accuracy with human-in-the-loop review." },
    ],
    status: "ready",
    note: "GenAI Insights bullet and Rogers & Shaw case study added from docs/Adarsh.docx (Aug 2026); Astound case study figures sharpened with the same source's 89% agent-retention and 20% team-leader-cost figures, previously left vague as \"a related engagement.\"",
  },
  ...
  // ---------------- Back-Office & Case Automation ----------------
  {
    ...
    variants: [{ label: "Default", video: null }],
    caseStudies: [
      { client: "CHC+ (Ealing Council, UK)", result: "Cut CHC eligibility-checklist drafting time from ~4 hours to ~30 minutes, took the rejection rate to zero across the pilot, and exceeded the annual CHC acceptance target — helping close an estimated £270M national CHC funding gap." },
    ...
    status: "needs-review",
    note: "Source deck's problem/solution slide (103) was phrased as solution highlights rather than pain points. Reframed into problem vs. what-you'll-see using context from the related back-office-accelerator narrative (slide 120). Case studies (CHC+, GDS) added from the HGS AgentX positioning doc — this is the closest match to AgentX's \"Case Worker Automation\" module. CHC+ figure enriched with the national funding-gap and acceptance-target detail from docs/Adarsh.docx (Aug 2026). Note: that same source frames the GDS case study as a \"Digital AI Assistant\" under Customer Engagement (webform/email intake with HITL escalation), not under Case Worker/Process Intelligence as filed here — left in place since the underlying capability (case intake, triage, and resolution) still matches this entry, but worth a look if Customer Engagement ever needs its own GDS-based demo.",
  },
  {
    id: "accounts-payable-automation",
    subcategory: "case-document-automation",
    title: "Accounts Payable Automation",
    tagline: "From invoice to payment, with people only in the exceptions",
    industryTags: ["cross-industry"],
    product: "HGS AgentX",
    problem: [
      "Invoice ingestion, matching, and approval cycles are manual, slow, and error-prone",
      "Manual processing drives cash leakage and vendor dissatisfaction",
      "Duplicate payments, pricing errors, and compliance gaps go unnoticed until after the fact",
    ],
    whatYouSee: [
      "Invoice Capture automatically extracts and classifies invoice data from any format, removing manual keying",
      "Matching & Validation checks invoices against purchase orders and receipts, catching discrepancies before payment",
      "Approval Workflow routes invoices automatically, with people approving only exceptions and high-value payments",
      "Leakage Detection continuously scans for duplicate payments, pricing errors, and compliance gaps",
    ],
    variants: [{ label: "Default", video: null }],
    status: "needs-review",
    note: "New entry sourced from docs/Adarsh.docx (Aug 2026), one of three named modules under Agentic Process Automation. No case study, demo video, or screenshot identified yet — doc gives problem/solution framing only.",
  },
  {
    id: "fraud-anomaly-detection",
    subcategory: "case-document-automation",
    title: "Fraud & Anomaly Detection",
    tagline: "Catch the drift before it becomes a bigger failure",
    industryTags: ["cross-industry"],
    product: "HGS AgentX",
    problem: [
      "Exceptions and risk signals stall in a human queue, reviewed only after the fact",
      "The same failure keeps recurring because nobody has time to fix the underlying process",
      "High false-positive rates and heavy manual investigation workload",
    ],
    whatYouSee: [
      "Risk Signal Monitoring continuously watches transactions and process activity for early warning signs",
      "Anomaly Detection flags unusual patterns and process drift automatically",
      "Case Triage & Resolution resolves routine, low-risk exceptions directly, routing only critical cases to people",
      "Audit & Governance Trail keeps a full, auditable record of every automated decision and action",
    ],
    variants: [{ label: "Default", video: null }],
    status: "needs-review",
    note: "New entry sourced from docs/Adarsh.docx (Aug 2026), one of three named modules under Agentic Process Automation — framed there as general enterprise process-risk monitoring (Risk Signal Monitoring, Anomaly Detection, Case Triage & Resolution, Audit & Governance Trail), broader than amlens's AML-specific alert-investigation-overlay framing for banks. Kept as a separate entry rather than merged into amlens since the two target different buyers and problem scopes; worth revisiting if they turn out to be the same underlying platform capability described two ways. No case study, demo video, or screenshot identified yet.",
  },
  {
    id: "claim-automation",
    subcategory: "case-document-automation",
